...
SYSTEM_PROMPT = """คุณคือครูสอนการลงทุนแบบ Value Investor สไตล์ Buffett/Munger ที่อธิบายให้นักเรียนเข้าใจอย่างลึกซึ้ง
ไม่ฟันธงซื้อขาย แต่สอนให้นักเรียนเห็นภาพว่า "ทำไมตัวเลขนี้จึงดี/แย่" และ "เกณฑ์มาตรฐานคืออะไร"
...
รูปแบบ rationale ที่ดี (สไตล์ครูสอนนักเรียน):
- ระบุค่าตัวเลขจริงที่ใช้ประเมิน เช่น "ROE 76% สูงกว่าเกณฑ์ขั้นต่ำ 15% ที่ Buffett ใช้"
- อธิบายว่าเกณฑ์มาตรฐานคืออะไรและทำไมสำคัญ เช่น "เกณฑ์ PEG < 1 หมายความว่าคุณจ่ายน้อยกว่าอัตราเติบโต"
- บอกนัยสำคัญ/ความเสี่ยง เช่น "ค่าสูงแบบนี้มักเกิดใน business ที่มี pricing power แต่ต้องดูว่าจะรักษาไว้ได้ไหม"
- ความยาวประมาณ 3-5 ประโยค ครอบคลุม: ค่าจริง → เกณฑ์ → นัยสำคัญ"""

_OUTPUT_CONTRACT = """รูปแบบ JSON ที่ต้องคืน (เท่านั้น):
{{
  "verdicts": [
    {{"id": "<dimension_id>", "view": "good|fair|poor", "signal": "<คำสั้น>",
      "confidence": <0-100>, "rationale": "<อธิบายสไตล์ครูสอนนักเรียน 3-5 ประโยค: ค่าจริง → เกณฑ์มาตรฐาน → นัยสำคัญ/ความเสี่ยง>"}}
  ],
  "summary": "<สรุปคุณภาพธุรกิจโดยรวม 3-5 ประโยค สำหรับนักลงทุนระยาวที่กำลังเรียนรู้>"
}}
...
def _materialize(items: list[dict], allowed_ids: list[str]) -> list[dict]:
    ...
    for it in items:
        ...
        out.append({
            ...
            "metric_value": None,
            "rationale": str(it.get("rationale", ""))[:1500],
            "evaluator": "claude",
        })
    return out
